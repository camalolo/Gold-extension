...
Manifest V3 badge-only extension. Displays the current gold (XAU) price in USD on the toolbar badge.
...
- **Badge display with abbreviation** — `updateBadge(priceText)` checks `chrome.storage.local` for an `abbreviation` flag. When enabled and price ≥ 1000, formats as e.g. `"1.2k"`; otherwise shows whole number (`toFixed(0)`). Truncates to 4 chars max.
...
- **Cache with TTL** — `fetchGoldPrice(forceUpdate=false)` checks `chrome.storage.local` for a cached value with 30-minute TTL before hitting the API.
- **API:** `GET https://api.gold-api.com/price/XAU` — extracts `data.price`.
...
- **Alarm** — `chrome.alarms` fires `'updateGoldPrice'` every 30 minutes.
- **Storage keys in `chrome.storage.local`:** `goldPrice` (number), `goldPriceTimestamp` (epoch ms), `abbreviation` (boolean).
...
- **Abbreviation flag** is read from `chrome.storage.local` but there's no options UI to toggle it — code pattern shared with Silver-extension.
